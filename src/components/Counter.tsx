...
function DecimalPointDigit({ height, digitStyle }: { height: number; digitStyle?: React.CSSProperties }) {
  return (
    <span
    ...
    <span style={{ ...defaultContainerStyle, ...containerStyle }}>
      <span style={{ ...defaultCounterStyle, ...counterStyle }}>
        {places.map((place, index) =>
          place === '.' ? (
            <DecimalPointDigit key={`dot-${index}`} height={height} digitStyle={digitStyle} />
          ) : (
            <NumericDigit key={place} place={place} value={value} height={height} digitStyle={digitStyle} />
          )
        )}
      </span>
